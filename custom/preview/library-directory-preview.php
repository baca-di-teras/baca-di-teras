<?php
/**
 * Library Directory Preview
 * Development-only preview for the integrated library directory page.
 */

$scriptName = $_SERVER['SCRIPT_NAME'] ?? '';
$baseUrl = str_replace('/custom/preview/library-directory-preview.php', '', $scriptName);
...
<head>
    ...
    <title>Preview - Library Directory</title>
    <style>
        html,
        body {
            min-height: 100%;
        ...
        .bdt-network-preview .bdt-hero-section {
            min-height: 270px;
        }

        .bdt-network-preview .bdt-hero-section__container {
            width: min(calc(100% - 100px), 1024px);
            padding: 40px 0 48px;
        }

        .bdt-network-preview .bdt-hero-section__label {
            gap: 5px;
            min-height: 20px;
            padding: 4px 10px;
            font-size: 10px;
        ...
        .bdt-network-preview .bdt-hero-section__label svg {
            width: 12px;
            height: 12px;
        }

        .bdt-network-preview .bdt-hero-section__title {
            max-width: 650px;
            margin-top: 22px;
            font-size: 42px;
            line-height: 1.12;
        ...
        .bdt-network-preview .bdt-hero-section__description {
            max-width: 560px;
            margin-top: 18px;
            font-size: 17px;
            line-height: 1.55;
        ...
        .bdt-network-preview__toolbar .bdt-search-bar {
            flex: 0 0 340px;
            height: 38px;
        }

        .bdt-network-preview__toolbar .bdt-search-bar__field {
            padding: 0 14px;
            border-width: 1px;
            border-radius: 5px;
        }

        .bdt-network-preview__toolbar .bdt-search-bar__icon {
            width: 17px;
            height: 17px;
            margin-right: 13px;
        }

        .bdt-network-preview__toolbar .bdt-search-bar__input {
            font-size: 12px;
        }

        .bdt-network-preview__toolbar .bdt-chip-filter {
            flex: 1 1 auto;
            min-height: 38px;
            padding: 0;
            border: 0;
        ...
        .bdt-network-preview__toolbar .bdt-chip-filter__item {
            min-height: 30px;
            padding: 0 16px;
            font-size: 12px;
            font-weight: 700;
...
<body class="bdt-network-preview">
    <?php include __DIR__ . '/../components/navbar.php'; ?>
    <?php include __DIR__ . '/../components/hero.php'; ?>

    <main class="bdt-network-preview__content">
        <div class="bdt-network-preview__toolbar">
            <?php include __DIR__ . '/../components/search-bar.php'; ?>
            <?php include __DIR__ . '/../components/chip-filter.php'; ?>
        </div>

        <section class="bdt-network-preview__grid" aria-label="Daftar perpustakaan desa">
