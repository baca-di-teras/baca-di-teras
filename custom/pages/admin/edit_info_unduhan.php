...
if (!$info || $info['id_name'] !== 'unduhan') { header("Location: " . BASE_URL . "/portal-admin/informasi"); exit; }
$extra = json_decode($info['extra_data'], true) ?: [];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $files = [];
    if (isset($_POST['download_label']) && is_array($_POST['download_label'])) {
        for ($i = 0; $i < count($_POST['download_label']); $i++) {
            $label = trim($_POST['download_label'][$i]);
            $href = trim($_POST['download_href'][$i] ?? '#');
            if ($label) $files[] = ['label' => $label, 'href' => $href];
        }
    }
    $data = [
        'id_name'    => 'unduhan',
        'type'       => 'download',
        'title'      => $_POST['title'] ?? 'Unduhan',
        'content'    => $_POST['content'] ?? '',
        'extra_data' => json_encode(['files' => $files]),
...
<body>
        ...
        <div class="admin-content" style="padding: 32px; max-width: 1200px; margin: 0 auto; width: 100%; box-sizing: border-box;">
            <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px;">
                <div class="page-title">
                    <h1 style="font-size: 1.5rem; font-weight: 700; margin: 0 0 8px 0;">Edit Unduhan</h1>
                </div>
            ...
            <form method="POST" class="card" style="padding: 32px; background: white; border-radius: 12px; border: 1px solid var(--admin-border);">
                ...
                <div class="form-group">
                    <label class="form-label">Judul Bagian Unduhan</label>
                    <input type="text" name="title" class="form-control" required value="<?= htmlspecialchars($info['title'] ?? 'Unduhan') ?>">
                </div>
                ...
                <h3 class="form-section-title">Daftar Dokumen</h3>
                <div class="form-group">
                    <div class="dynamic-list" id="downloadList">
                        <?php $files = $extra['files'] ?? [['label' => '', 'href' => '']]; ?>
                        <?php foreach ($files as $file): ?>
                        <div class="dynamic-item">
                            <input type="text" name="download_label[]" class="form-control" value="<?= htmlspecialchars($file['label']) ?>" placeholder="Nama Dokumen">
                            <input type="text" name="download_href[]" class="form-control" value="<?= htmlspecialchars($file['href']) ?>" placeholder="URL File">
                            <button type="button" class="btn-remove" onclick="this.parentElement.remove()">Hapus</button>
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <button type="button" class="btn-add" onclick="addDownload()">+ Tambah Dokumen</button>
                </div>
    ...
    <script>
        function addDownload() {
            ...
            div.innerHTML = `<input type="text" name="download_label[]" class="form-control" placeholder="Nama Dokumen"><input type="text" name="download_href[]" class="form-control" placeholder="URL File"><button type="button" class="btn-remove" onclick="this.parentElement.remove()">Hapus</button>`;
