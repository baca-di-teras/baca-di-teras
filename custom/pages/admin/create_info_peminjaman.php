...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $max = $db->fetchScalar("SELECT MAX(sort_order) FROM bdt_information WHERE id_name='peminjaman_point'");
...
<head>
    <meta charset="UTF-8">
    <title>Tambah Poin Peminjaman</title>
        ...
        <div class="admin-content" style="padding: 32px; max-width: 1200px; margin: 0 auto; width: 100%; box-sizing: border-box;">
            <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px;">
                <div class="page-title">
                    <h1 style="font-size: 1.5rem; font-weight: 700; margin: 0 0 8px 0;">Tambah Poin Peminjaman</h1>
                </div>
            ...
            <form method="POST" class="card" style="padding: 32px; background: white; border-radius: 12px; border: 1px solid var(--admin-border);">
                ...
                <div class="form-group">
                    <label class="form-label">Teks Poin Aturan</label>
                    <input type="text" name="title" class="form-control" required placeholder="Misal: Maksimal 3 buku per anggota" value="<?= htmlspecialchars($info['title'] ?? '') ?>">
                </div>
